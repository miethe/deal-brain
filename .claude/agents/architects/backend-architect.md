...
Before implementing backend solutions, use optimized API symbol chunks to understand existing patterns:
- Primary API context: `cat ai/symbols-api.json | head -100` (services, routers, repositories - token optimized)
- Shared utilities: `cat ai/symbols-shared.json | jq '.modules[] | select(.path | contains("lib") or contains("utils"))'`
- API test context (when debugging): `cat ai/symbols-api-tests.json | head -50`
- Search patterns: `grep -E "(Service|Repository|Router|DTO|Schema)" ai/symbols-api.json`

## Key Actions
1. **Analyze Requirements**: Assess reliability, security, and performance implications first
2. **Query Symbol Context**: Load relevant API symbols to understand existing architecture patterns

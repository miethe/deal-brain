...
from dealbrain_core.enums import ComponentMetric, ComponentType, Condition, RamGeneration, StorageMedium
...
from ..models import Cpu, Gpu, Listing, ListingComponent, Profile, RamSpec, StorageProfile
...
def _normalize_ram_generation(value: Any) -> RamGeneration:
    if isinstance(value, RamGeneration):
        return value
    normalized = (_normalize_str(value) or "").lower()
    if not normalized:
        return RamGeneration.UNKNOWN
    for generation in RamGeneration:
        if generation.value == normalized:
            return generation
        if generation.name.lower() == normalized:
            return generation
...
def _normalize_ram_spec_payload(
    ...
    *,
    ...
    generation = _normalize_ram_generation(
        data.get("ddr_generation")
        or data.get("ram_type")
        or data.get("generation")
        or fallback_generation
    )
    speed_mhz = _normalize_int(data.get("speed_mhz") or data.get("speed") or fallback_speed)
    module_count = _normalize_int(data.get("module_count") or data.get("modules"))
    capacity_per_module = _normalize_int(
        data.get("capacity_per_module_gb")
        or data.get("module_capacity_gb")
        or data.get("capacity_per_module")
    )
    total_capacity = _normalize_int(
    ...
    label = _normalize_str(data.get("label")) or _format_ram_label(
        generation,
        speed_mhz,
        total_capacity,
        module_count,
    ...
    notes = _normalize_str(data.get("notes"))
...
async def _get_or_create_ram_spec(session: AsyncSession, spec_data: dict[str, Any]) -> RamSpec:
    normalized = _normalize_ram_spec_payload(spec_data, fallback_total_gb=spec_data.get("total_capacity_gb"))
    if not normalized:
    ...
    ram_spec = RamSpec(
        label=label or _format_ram_label(
...
def _normalize_storage_medium(value: Any) -> StorageMedium:
...
def _normalize_storage_profile_payload(
    data: dict[str, Any] | None,
    *,
    ...
    medium = _normalize_storage_medium(
        data.get("medium")
        or data.get("storage_type")
        or fallback_type
    )
    interface = _normalize_str(data.get("interface") or data.get("bus"))
    form_factor = _normalize_str(data.get("form_factor") or data.get("size"))
    capacity_gb = _normalize_int(
        data.get("capacity_gb") or data.get("capacity") or data.get("storage_gb") or fallback_capacity_gb
    )
    performance_tier = _normalize_str(data.get("performance_tier") or data.get("tier"))
    if capacity_gb is None:
        return None

    attributes = data.get("attributes") or data.get("attributes_json")
    attributes_json = attributes if isinstance(attributes, dict) else {}
    label = _normalize_str(data.get("label")) or _format_storage_label(medium, capacity_gb, interface, form_factor)
    notes = _normalize_str(data.get("notes"))
...
async def _get_or_create_storage_profile(session: AsyncSession, profile_data: dict[str, Any]) -> StorageProfile:
    normalized = _normalize_storage_profile_payload(profile_data, fallback_capacity_gb=profile_data.get("capacity_gb"))
    if not normalized:
    ...
    storage_profile = StorageProfile(
        label=label or _format_storage_label(
            normalized["medium"],
...
def _storage_medium_display(medium: StorageMedium) -> str | None:
    mapping = {
...
async def _prepare_component_relationships(
    ...
    *,
    ...
    if "ram_spec_id" not in payload:
        total_capacity = _normalize_int(payload.get("ram_gb"))
        ...
        spec_input = _normalize_ram_spec_payload(
            ram_spec_payload,
            fallback_total_gb=total_capacity,
            fallback_generation=_normalize_ram_generation(ram_type_hint) if ram_type_hint else (
                listing.ram_spec.ddr_generation if listing and listing.ram_spec else None
            ),
            fallback_speed=_normalize_int(ram_speed_hint) or (
                listing.ram_spec.speed_mhz if listing and listing.ram_spec else None
            ),
        )
        if spec_input:
            ram_spec = await _get_or_create_ram_spec(session, spec_input)
    ...
    for prefix in ("primary", "secondary"):
        ...
        fallback_capacity = _normalize_int(payload.get(capacity_key))
        if fallback_capacity is None:
            if existing_profile and existing_profile.capacity_gb is not None:
                ...
                fallback_capacity = _normalize_int(getattr(listing, f"{prefix}_storage_gb", None))
        ...
        profile_input = _normalize_storage_profile_payload(
            profile_payload,
        ...
        storage_profile = await _get_or_create_storage_profile(session, profile_input)
        ...
        if storage_profile.medium and storage_profile.medium != StorageMedium.UNKNOWN:
            payload[storage_type_key] = payload.get(storage_type_key) or _storage_medium_display(storage_profile.medium)

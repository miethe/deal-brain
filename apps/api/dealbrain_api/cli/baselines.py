...
def load_baseline(
    source: Path = typer.Argument(
        ..., exists=True, readable=True, help="Path to baseline JSON artifact"
    ),
    actor: str = typer.Option(
        "system", "--actor", "-a", help="Actor recorded in metadata/audit logs"
    ),
    ...
    async def _run() -> dict[str, object]:
        async with session_scope() as session:
            ...
            result = await service.load_from_path(
                ...
                source,
                actor=actor,
                ensure_basic_for_ruleset=(
                    ensure_basic_ruleset_id if ensure_basic_ruleset_id > 0 else None
                ),
            )

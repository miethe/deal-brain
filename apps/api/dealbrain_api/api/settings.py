...
async def get_setting(key: str, session: AsyncSession = Depends(session_dependency)):
...
async def update_setting(
    key: str, request: SettingUpdateRequest, session: AsyncSession = Depends(session_dependency)
):
    """Update or create a setting."""
    service = SettingsService()
    value = await service.update_setting(session, key, request.value, request.description)
    return value


@router.get("/valuation_thresholds/default", response_model=dict[str, float])
async def get_valuation_thresholds(session: AsyncSession = Depends(session_dependency)):

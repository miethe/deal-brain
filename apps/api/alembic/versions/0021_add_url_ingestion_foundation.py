...
def upgrade():
    ...
    op.add_column(
        "listing", sa.Column("marketplace", sa.String(16), nullable=False, server_default="other")
    )
    ...
    op.create_unique_constraint(
        "uq_listing_vendor_id", "listing", ["vendor_item_id", "marketplace"]
    )
    ...
    op.add_column(
        "import_session",
        sa.Column("source_type", sa.String(16), nullable=False, server_default="excel"),
    )
    ...
    op.create_index(
        "ix_ingestion_metric_adapter_measured", "ingestion_metric", ["adapter", "measured_at"]
    )

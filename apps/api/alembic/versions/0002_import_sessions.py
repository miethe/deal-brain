...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "sheet_meta_json", sa.JSON(), nullable=False, server_default=sa.text("'{}'::jsonb")
        ),
        sa.Column(
            "mappings_json", sa.JSON(), nullable=False, server_default=sa.text("'{}'::jsonb")
        ),
        sa.Column(
            "conflicts_json", sa.JSON(), nullable=False, server_default=sa.text("'{}'::jsonb")
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.ForeignKey("import_session.id", ondelete="CASCADE"),
            nullable=False,
        ),
